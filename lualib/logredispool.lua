...
local redis_pwd = skynet.getenv("log_redis_pwd")
if redis_pwd == "" then
	redis_pwd = nil
end

local conf = {
		host = skynet.getenv("log_redis_host") ,
		port = skynet.getenv("log_redis_port") ,
		auth = redis_pwd ,
	}
...
function CMD.start()
	maxconn = tonumber(skynet.getenv("log_redis_maxconn")) or 3
	assert(maxconn >= 2)
