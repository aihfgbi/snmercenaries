...
char* get_log_filename(const char* basename)
{
	...
	snprintf(filename, sizeof(filename), "%s%s-%d.log", basename, timebuf, inst.roll_index++);
	return filename;
}
...
void rollfile()
{
	...
	while (1)
	{
		snprintf(filename, sizeof(filename), "%s/%s", inst.dirname, get_log_filename(inst.basename));
...
void append_log(const char* log, int level)
{
	...
	snprintf(msg, sizeof(msg), "[%s.%3.3lu] %s %s\n", timebuf, tv.tv_usec/1000, log_level_str(level), log);
