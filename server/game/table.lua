...
local function leave(p, reason, win)
	if not p or not p.uid then return end
	if not players or not players[p.uid] then return end
	-- reason = reason or 1
	...
	if config.max_player < 9 then
		-- 如果是百人场，那么不需要通知给其他人
		api.send_to_all("game.LeaveTableNtf", {uid=p.uid, result = reason})
	else
		p:send_msg("game.LeaveTableNtf", {uid=p.uid, result = reason})
	end
	if p.isrobot then
		p:send_msg("exit")
	end
...
function CMD.join(p)
	...
	if result then
		...
		api.send_except("game.EnterTable", {uid=p.uid, nickname=p.nickname, sex = p.sex or 1, seatid = p.seatid,
			online = 1, ready = p.ready, score = p.score, gold = p.gold, money=p.hongbao or 0, headimg=p.headimg or ""}, p.uid)
		logic.resume(p)
	else
		return 1008
	end
...
function CMD.dispatch(uid, name, msg)
	...
	if name == "ChatNtf" then
		...
		if msg.type == 1 or msg.type == 2 then
			local info = tonumber(msg.msg)
			if info and info >= 0 and info < 50 then
		...
		elseif msg.type == 3 then
			...
			if msgid and expression_cfg[msgid] and expression_cfg[msgid].cost then
				local p = players[uid]
				if p then
					local sub_gold_ret = p:call_userdata("sub_gold", p.uid, expression_cfg[msgid].cost, 1002)
					if sub_gold_ret then
						api.send_to_all("game.ChatNtf", {type=msg.type,to=msg.to,msg=msg.msg,uid=uid})
					else
...
function api.get_robot(type, gold)
	...
	if robotmanager then 
		local ok, robot = pcall(cluster.call, "robot", robotmanager, "get_robot", type, gold or 0, gameid, nodename, skynet.self())
...
function api.game_end(result)
	...
	if result and histroy then
		local data
		local now = os.time()
		if not record then now = 0 end --如果不需要录像，那么是没有hash的
		for uid,p in pairs(players) do
