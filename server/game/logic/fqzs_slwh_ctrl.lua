...
--判断是否控制起效
local function juge_player_ctrl(p, pour_message, win_message)
	--数据赋值
	local rate = math.random(1,100)
	local now_ctrlgold = p.ctrlinfo.ctrlnowgold
	--判断赋值
	if p.ctrlinfo.ctrltype == 1 then
		now_ctrlgold = now_ctrlgold - (win_message[p.uid] - pour_message[p.uid])
	else
		now_ctrlgold = now_ctrlgold + (win_message[p.uid] - pour_message[p.uid])
	end
	--判断赋值
	...
	elseif p.ctrlinfo.ctrltype == 1 and p.ctrlinfo.ctrlnowgold > p.ctrlinfo.ctrlmaxgold then
		...
		if rate <= p.ctrlinfo.ctrlrate then
			if p.ctrlinfo.ctrltype == 1 and (win_message[p.uid] < pour_message[p.uid]) then
				if p.ctrlinfo.ctrlcount and p.ctrlinfo.ctrlcount > 0 then
				...
				return true
			elseif p.ctrlinfo.ctrltype == 2 and (win_message[p.uid] > pour_message[p.uid]) then
				if p.ctrlinfo.ctrlcount and p.ctrlinfo.ctrlcount > 0 then
					p.ctrlinfo.ctrlcount = p.ctrlinfo.ctrlcount - 1
					if p.ctrlinfo.ctrlcount == 0 then
						p.ctrlinfo.ctrlcount = nil
...
--第一次判断控制
local function check_open_first(t_bet, t_win, pour_message, win_message)
	-------------------单人控制判断------------------
	local ctrl_player = {}
	for uid,p in pairs(players) do
		if not p.isrobot then
			if p.ctrlinfo and p.ctrlinfo.ctrltype and p.ctrlinfo.ctrllevel then
				if pour_message[uid] and pour_message[uid] > 0 or win_message[uid] ~= 0 then
	...
	for i=12,1,-1 do
		if ctrl_player[i] then
			if #ctrl_player[i] == 1 then
				local player_ctrl = juge_player_ctrl(players[ctrl_player[i][1]], pour_message, win_message)
				--判断显示
				...
				for k,uid in pairs(ctrl_player[i]) do
					--判断冲突
					if juge_player_ctrl(players[uid], pour_message, win_message) == false then
						is_open = false
						break
					end
				end
...
--检测是否开牌
function this.check_open(_times, pour_message, win_message)
	local t_bet,t_win = 0,0
	for uid,money in pairs(pour_message) do
		t_bet = t_bet + (money or 0)
	end
	for uid,money in pairs(win_message) do
		t_win = t_win + (money or 0)
	end
	for uid,p in pairs(players) do
		if not p.isrobot then
			if p.ctrlinfo and p.ctrlinfo.ctrltype and p.ctrlinfo.ctrllevel then
				pour_message[uid] = pour_message[uid] or 0
				win_message[uid] = win_message[uid] or 0
			end
		end
	end
	--判断返回
	...
	elseif _times == 0 then
		return check_open_first(t_bet, t_win, pour_message, win_message)
	else
		if ctrl_type == 1 then
			if ctrl_result == 1 then
				if win_message[ctrl_user] <= pour_message[ctrl_user] then
					return true
				else
					return false
				end
				...
				if players[ctrl_user].ctrlinfo.ctrltype == 1 then
					now_ctrlgold = now_ctrlgold - (win_message[ctrl_user] - pour_message[ctrl_user])
					...
					if now_ctrlgold > players[ctrl_user].ctrlinfo.ctrlmaxgold then
						...
						if win_message[ctrl_user] - pour_message[ctrl_user] >= 0  then
							return true
						else
							return false
						end
