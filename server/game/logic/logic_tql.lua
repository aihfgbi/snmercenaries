...
local pour_money=3000   --每一注的下注金额
...
local random_time_lock = 100 --免费次数的限额
local cardpool = assert(require "tql_pool")

local freetime={}   --免费次数阶梯
local gold_rate={1,2,3,5,10,20,30,60,120}
local random_rate = {[3]=20,[4]=20,[5]=30,[6]=10,[7]=10,[8]=10}
local pri_gold = 3000  --1倍的金币数额

local TimeCD = 1   --每一局之间的时间间隔
local wintype_X    --金球位置的table

local had_win=false          --控制器控制中奖是否已经中了
local win_count=5            --如果控制器控制已经中了，在5次后had_win会变为false
--定义水果顺序 J1，Q2，K3，A4，鸡尾酒5，耳机6，CD7，钻石8，JUMP9，金球10，
--重新定义 各种倍率及免费的概率
-- local numberChance={[1]=40,[2]=40,[3]=30,[4]=30,[5]=25,[6]=20,[7]=15,[8]=10,[9]=5,[10]=20 or 5}
local numberChance={[0]=250,[5]=30,[6]=30,[8]=30,[10]=80,[12]=70,[15]=50,[20]=50,[25]=40,[30]=40,
	[40]=30,[50]=40,[60]=20,[70]=20,[80]=20,[90]=10,[100]=10,[150]=10,[175]=10,[200]=5,[250]=2,[1000]=10 or 1}   --1000代表免费  其余代表倍率
...
local function CheckWin()    --检测是否符合要求
	...
	if randomX <= min_number then 
		...
		elseif min_number == 50000 and currentwin <= sysearn*sysearn_rate then   --不需要控制
			...
			LOG_WARNING("不符合要求,次数是"..re_count.."  系统收入："..sysearn)
			return false
		end		
	end
...
local function addfree(addrandomX)
...
local function RandomGold()
	...
	if count_qiu >= 5 then
		...
		local addrandomX=13   --基本的免费次数
		if randomtime + addrandomX > random_time_lock then 
			addrandomX= random_time_lock - randomtime
		end
		
		local sum = 0
		for k,v in pairs(random_rate) do
			sum = sum + v
		end
		local rate_random = math.random(1,sum)
		local x_match = 0
		for k,v in pairs(random_rate) do
			x_match = x_match + v
			if x_match >= rate_random then
				addrate = k
				break
			end
		end
...
local function SumTotal()
	-- PRINT_T(numberChance)
	total = 0
	for i,v in pairs(numberChance) do
		total=total+v
	end
	-- LOG_WARNING("total="..total)
end
...
function random(p)
	...
	for m,v in pairs(numberChance) do      --对随机出来的数字进行查找 
		...
		if sum>=k then	
			...
			local randomx		
			if m ~= 1000 then
				randomx = math.random(1,table.len(cardpool.rate[m]))
				showcards = table.deepcopy(cardpool.rate[m][randomx]) 
			elseif m == 1000 then
				randomx = math.random(1,table.len(cardpool.free))
				showcards = table.deepcopy(cardpool.free[randomx]) 
			end				
			break
		end
	end
	-- PRINT_T(showcards)
	local copy_cards= table.deepcopy(showcards)
	-- PRINT_T(copy_cards)
	for i=1,5 do
		local current_line={}
		for j=1,3 do
			local x = table.remove(copy_cards,1)
			-- LOG_DEBUG((3*(i-1)+j)..":"..x)
			tinsert(current_line,x)
			if x == 10 then
				wintype_X[3*(i-1)+j]=1
				count_qiu = count_qiu + 1
			end
		end
		tinsert(randomnumber,current_line)
	end
	-- PRINT_T(randomnumber)
	if count_qiu < 5 then
		wintype_X = nil
	end
			...
			if (p.gold>=pour_money and pour_money>= lock_pourmoneydown) or randomtime>0 then
				if randomtime > 0 then 
					...
					if current_free > 0 then
						table.remove(freetime,current_free)
					end
...
function this.init(ps, api, m_conf, m_times, m_score, m_pay, m_code, m_gameid, uid, usegold, _, params, kb)
	...
	if config.test_gold then
		...
		-- LOG_WARNING("体验房")
		if config.numberChance then
			numberChance=config.numberChance
			-- LOG_WARNING("修改概率")
		end
	end
	...
	if config.init_params.gold_rate then
		gold_rate = config.init_params.gold_rate
	end
