...
local function clear_invite_list()
    if not userdata.group or not userdata.group.child then
        return
    end
...
function this.BuyHongbaoReq(msg)
    ...
    if not cfgWinHongbao[msg.id] then
...
function this.BuyRealGoodsReq(msg)
    ...
    if not cfg then
        return "user.BuyRealGoodsRep", {result = 1003, phone = msg.phone}
    end
    if cfg.type ~= 5 then
        return "user.BuyRealGoodsRep", {result = 1004, phone = msg.phone}
    end
...
function this.BuyReq(msg)
    if not msg or not msg.id or not msg.count then
        return
    end
...
function this.JoinMatch(msg)
    if not msg or not msg.matchid then
        return
    end
    if gamenode then
...
function this.SingleHistroyReq(msg)
    if not msg or not msg.index then
        return
    end
    if msg.index > 50 or msg.index < 0 then
        return
    end
...
function this.CheckPassReq(msg)
    -- 做验证限制，防止出错
    if not msg or not msg.pass then
        return
    end
    if not userdata.safepass then
        return "user.CheckPassRep", {result = 1000}
    end
    if userdata.safepass ~= msg.pass then
        return "user.CheckPassRep", {result = 1001}
    end
    return "user.CheckPassRep", {result = 0}
end

function this.ResetSafePasswordReq(msg)
    if not msg or not msg.oldpass or not msg.newpass then
        return
    end
...
function this.SafeMoneyReq(msg)
    ...
    if not userdata.safepass then
        return "user.SafeMoneyRep", {value = -2}
    end
    if userdata.safepass ~= msg.pass then
        return "user.SafeMoneyRep", {value = -1}
    end
...
function this.SaveSafeMoneyReq(msg)
    if not msg or not msg.value or not msg.pass then
        return
    end
    if not userdata.safepass then
        return "user.SaveSafeMoneyRep", {result = 1000}
    end
    if userdata.safepass ~= msg.pass then
        return "user.SaveSafeMoneyRep", {result = 1001}
    end
    if gamenode then
        -- 在游戏中，不能存钱，请稍候再试
    ...
    if sub_gold(value, 106) then
        if add_bank(value, 1001) then
            local sql =
                ...
                os.date("%Y%m") ..
                    "(Uid,Type,Create_Time,Value) VALUES(" ..
            ...
            log(
                "save_safe_money",
                string.format(
                    '{"gold":%d,"money":%d,"bank":%d,"type":%d,"value":%d}',
                    userdata.gold or 0,
                    userdata.money or 0,
                    userdata.bank or 0,
                    1,
...
function this.TakeSafeMoneyReq(msg)
    if not msg or not msg.value or not msg.pass or not msg.channel then
        return
    end
    if not userdata.safepass then
        return "user.TakeSafeMoneyRep", {result = 1000}
    end
    if userdata.safepass ~= msg.pass then
        return "user.TakeSafeMoneyRep", {result = 1001}
    end
    local value = math.floor(tonumber(msg.value))
    if not value or value < 1 then
        return "user.TakeSafeMoneyRep", {result = 1002}
    end
    userdata.bank = userdata.bank or 0
    if value > userdata.bank then
...
function this.SafeHistroyReq(msg)
    -- 保险箱历史记录
    if not msg or not msg.year or not msg.month then
        return
    end
    if not userdata.safepass then
...
function this.BindParentNtf(msg)
    if not msg or not tonumber(msg.parent) then
        return
    end
    if tonumber(msg.parent) == uid then
        return
    end
    -- bind_parent
